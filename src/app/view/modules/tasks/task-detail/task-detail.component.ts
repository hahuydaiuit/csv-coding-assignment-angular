...
export class TaskDetailComponent implements OnInit {
  ...
  constructor(private readonly formBuilder: UntypedFormBuilder, public route: ActivatedRoute, private taskService: TasksService, public router: Router, private snackBarService: SnackBarService) { }
  ...
  initData() {
    const taskDetail = this.taskService.getTaskDetail(this.taskId).pipe(
      catchError(error => throwError(error))
      );
    const users = this.taskService.getAllUsers().pipe(
      catchError(error => throwError(error))
      );

    forkJoin([taskDetail, users]).subscribe((results) => {
  ...
  onSave() {
    this.taskService.updateTask(this.taskForm.value).subscribe((task) => {
      if (task) {
        this.router.navigate(['/tasks'])
      }
    }, (error) => {
